...
public function new($amount, $charge, $final_amount, $method, $description, string|TxnType $type, string|TxnStatus $status = TxnStatus::Pending, $payCurrency = null, $payAmount = null, $userID = null, $relatedUserID = null, $relatedModel = 'User', array $manualFieldData = [], $walletType = 'default',  $referenceId = null ,$card_id = null, string $approvalCause = 'none', $targetId = null, $targetType = null, $isLevel = false): Transaction
{
    if ($type === 'withdraw') {
        self::withdrawBalance($amount);
    }
    ...
    $transaction->wallet_type = $walletType;
    $transaction->card_id = $card_id;
    $transaction->reference_id = $referenceId;  // Save the reference_id to the database
    $transaction->save();
    ...
    // public static function transfer($amount, $charge, $final_amount, $description, string|TxnType $type, string|TxnStatus $status, $payCurrency, $payAmount, $userID, $relatedUserID, $relatedModel, $beneficiaryId, $purpose, $transferType, array $manualFieldData = [], $walletType = 'default')
    // {
    //     $transaction = new Transaction();
    //     $transaction->user_id = $userID ?? Auth::user()->id;
    //     $transaction->from_user_id = $relatedUserID;
...
public static function transfer(
    ...
    $walletType = 'default', 
    $referenceId = null , // New parameter
    $transferFrom = null // Add new parameter
)
{
    $transaction = new Transaction();
    $transaction->user_id = $userID ?? Auth::user()->id;
    ...
    $transaction->from_model = $relatedModel;
    $transaction->tnx = 'TRX'.strtoupper(Str::random(10));
    $transaction->reference_id = $referenceId; // Save reference ID
    $transaction->description = $description;
    $transaction->amount = $amount;
    $transaction->type = $type;
    $transaction->charge = $charge;
    ...
    $transaction->manual_field_data = json_encode($manualFieldData);
    $transaction->wallet_type = $walletType;
    $transaction->transfer_from = $transferFrom; // Save the transfer_from field
    $transaction->save();
