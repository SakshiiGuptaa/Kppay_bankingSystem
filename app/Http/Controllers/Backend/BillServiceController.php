...
class BillServiceController extends Controller
{
    ...
    private function insertService($data, $request)
    {
        ...
        }elseif($request->method  == 'reloadly'){
            BillService::updateOrCreate([
                'api_id' => $data['id'],
            ], [
                'name' => $data['name'],
                ...
                'data' => json_encode($data),
                'currency' => $data['localTransactionCurrencyCode'],
                'min_amount' => $data['minLocalTransactionAmount'],
                'max_amount' => $data['maxLocalTransactionAmount'],
                'method' => $request->method,
                'type' => $request->category,
                'label' => 'subscriberAccountNumber',
                'status' => true,
            ]);
        }
