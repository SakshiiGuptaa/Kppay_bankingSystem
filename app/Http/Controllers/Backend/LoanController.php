...
class LoanController extends Controller
{
    ...
    public function updateStore(Request $request, $loan_id)
    {
        $loan = Loan::findOrFail($loan_id);

        // Validate and process the request
        $request->validate([
            'installment_rate' => 'required|numeric',
            'installment_intervel' => 'required|numeric',
            'total_installment' => 'required|numeric',
            'loan_amount' => 'required|numeric',
        ]);
    
        $loan->update([
            'installment_rate' => $request->installment_rate,
            'installment_interval' => $request->installment_intervel,
            'total_installments' => $request->total_installment,
    ...
    public function approvalAction(Request $request)
    {
        ...
        $shortcodes = [
            ...
            '[[given_installment]]' => 0,
            '[[total_installment]]' => $loan->total_installments,
