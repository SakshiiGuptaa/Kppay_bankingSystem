...
class WithdrawController extends Controller
{
    ...
    public function history(Request $request)
    {
        ...
        // Query to calculate total charges where status is 'success'
        $totalCharges = Transaction::whereIn('type', [TxnType::Withdraw->value, TxnType::WithdrawAuto->value])
                        ->where('status', 'success') // Filter only successful withdrawals
                        ->when($search, function($query) use ($search) {
                            $query->search($search);
                        })
                        ->sum('charge');
    
        return view('backend.withdraw.history', compact('withdrawals', 'totalCharges'));
    }
