...
@section('content')
    ...
    <div class="site-card-header d-flex" style="justify-content: right !important; gap: 10px;">
        <div class="title-small"></div>
        <a href="javascript:void(0)" class="site-btn-sm mb-4" data-bs-toggle="modal"
            data-bs-target="#selectWalletModal" style="background-color:#e71d1d; color:white;">
            <i data-lucide="plus-circle"></i>
            {{ __('Select Default Wallet') }}
        </a>
        ...
        <div class="user-profile-card">
            ...
            <h4 class="title">{{ __('Default Wallet ') }} - {{ $default_wallet->currency->name . "(".$default_wallet->currency->code}}) </h4>
                ...
                <div class="buttons mt-120">
                    <a href="{{ route('user.deposit.amount', $default_wallet?->currency?->code) }}" class="send me-2">
                        <i data-lucide="plus-circle"></i>{{ __('Add Money') }}
                    </a>
                    <a href="{{ route('user.fund_transfer.index', $default_wallet?->currency?->code) }}"  class="add">
                        <i data-lucide="send"></i>{{ __('Send Money') }}
                    </a>
                </div>
            @else
            <h4 class="title">{{ __('Default Wallet') }} </h4>
                {{-- Display default USD box --}}
                <h3 class="acc-balance" id="passo">
                    {{ setting('currency_symbol', 'global') . " ". number_format(0, 2) }}
                </h3>
                <!--<p>{{ __('Currency: USD') }}</p>-->
                <div class="buttons mt-120">
                    <a href="{{ route('user.deposit.amount')}}" class="send me-2">
                        <i data-lucide="plus-circle"></i>{{ __('Add Money') }}
                    </a>
                    <a href="{{ route('user.fund_transfer.index') }}"  class="add">
                        <i data-lucide="send"></i>{{ __('Send Money') }}
                    </a>
                </div>
            @endif
    ...
    <div class="modal fade" id="selectWalletModal" tabindex="-1" aria-labelledby="openTicketModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md modal-dialog-centered">
            <div class="modal-content site-table-modal">
                <div class="modal-body popup-body"> <button type="button" class="modal-btn-close" data-bs-dismiss="modal" aria-label="Close"> <i data-lucide="x"></i> </button>
                    <div class="popup-body-text">
                        <div class="title">{{ __('Select Default Wallet') }}</div>

                        <form action="{{ route('user.wallets.default') }}" method="post">
                            @csrf
                            <div class="step-details-form">
                                <div class="row">
                                    <div class="col-xl-12 col-lg-12 col-md-12">
                                        <div class="inputs">
                                            ...
                                            <select class="add-priority box-input page-count" name="wallet_id" >
                            ...
                            <div class="action-btns">
                                <button type="submit" class="site-btn-sm primary-btn me-2">
                                    <i data-lucide="check"></i> {{ __('Create Default Wallet') }}
                                </button>
                                        ...
                                        <div class="inputs">
                                            ...
                                            <select class="add-priority box-input page-count" name="currency" >
                                                ...
                                                @foreach ($currencies as $currency)
                                                    <option value="{{ $currency->id }}">{{ $currency->name." -> ".$currency->country_name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
